...
create_plot_game_involvement <- function(weekly_data_clean)
{
    set.seed(12345)
    library(ggplot2)

    pal <- paletteer::paletteer_d("DresdenColor::paired")
    ...
    plot_game_involvement <-
        ggplot(subset(gc_long, gameId %in% "49212"),
               mapping = aes(
                   x = reorder(name, -Number, sum),
                   y = Number,
                   fill = GC
               )) +
        geom_bar(position = "stack", stat = "identity") +
        scale_fill_manual(values = c("#785596FF", "#513965FF"),
                          name = "") +
...
create_plot_turnovers <- function(weekly_data_clean)
{
    set.seed(12345)
    library(ggplot2)
    # Turnovers
    turnovers_long <-
        tidyr::pivot_longer(
            weekly_data_clean,
            cols = c(turnovers, not_turnovers),
            names_to = "Turnovers",
            values_to = "Number"
        )
    ...
    plot_turnovers <-
        ggplot(
            subset(turnovers_long, gameId %in% "49212"),
            mapping = aes(
                x = reorder(name, -Number, sum),
                y = Number,
                fill = Turnovers
        ...
        geom_bar(position = "stack", stat = "identity") +
        scale_fill_manual(values = c("#C2697FFF", "#9E4058FF"),
                          name = "") +
...
# Team Contributions
create_plot_team_contribution <- function(weekly_data_clean)
{
    set.seed(12345)
    library(ggplot2)
    plot_team_contribution <-
        ggplot(
            subset(weekly_data_clean, gameId %in% "49212"),
        ...
        ggrepel::geom_text_repel(box.padding = 0.5,
                        force = 5,
                        segment.alpha = 0) +
        theme_classic() +
